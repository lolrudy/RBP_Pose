...
img_list_path = os.path.join(data_dir, 'CAMERA/val_list.txt')
...
for img_path in tqdm(img_list):
    img_path = os.path.join(data_dir, 'CAMERA', img_path)

    scene = img_path.split('/')[-2]
    img_id = img_path.split('/')[-1]
    detection_file = os.path.join(detection_dir, dataset_split, f'results_val_{scene}_{img_id}.pkl')
    ...
    detection_file_refine_mug = os.path.join(detection_dir_refine_mug, dataset_split, f'results_val_{scene}_{img_id}.pkl')
