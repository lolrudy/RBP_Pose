...
class prop_rot_loss(nn.Module):
    ...
    def forward(self, namelist, pred_list, gt_list, sym):
        ...
        if 'Prop_point_cano' in namelist:
            loss_list['Prop_point_cano'] = FLAGS.prop_p_w * self.cal_geo_loss_point(gt_list['Points'],
                                                                             pred_list['Rot1'],
                                                                             pred_list['Rot2'],
                                                                             pred_list['Tran'],
                                                                             gt_list['R'],
                                                                             gt_list['T'], sym, gt_list['Point_mask'])
    ...
    def prop_sym_matching_loss(self, PC, PC_re, p_g_vec, p_r_vec, p_t, gt_R, gt_t, sym, point_mask):
        bs = PC.shape[0]
        ...
        for i in range(bs):
            ...
            PC_re_now = PC_re[i, ...][point_mask_now]
            PC_re_cano = points_re_cano[i, ...][point_mask_now]
            p_g_now = p_g_vec[i, ...]
    ...
    def cal_geo_loss_point(self, points, p_rot_g, p_rot_r, p_t, g_R, g_t, sym, point_mask):
        '''
        ...
        # filter out invalid point
        points_re_y = torch.where(point_mask, points_re_y, torch.zeros_like(points_re_y))
        points_re[:, :, 0] = torch.where(point_mask, points_re[:, :, 0], torch.zeros_like(points_re_y))
        points_re[:, :, 1] = torch.where(point_mask, points_re[:, :, 1], torch.zeros_like(points_re_y))
        ...
        for i in range(bs):
            if sym[i, 0] == 1:
                ...
                points_re_x = torch.where(point_mask[i], points_re_x, torch.zeros_like(points_re_x))
