...
with open(os.path.join('/data/zrd/datasets/NOCS', 'obj_models/real_test.pkl'), 'rb') as f:
...
def find_best_match_for_gt(pred_result, refine_mug=False, img_path=None):
    ...
    for j in range(len(pred_result['gt_class_ids'])):
        ...
        if refine_mug and gt_cls_id == 6:
            ...
            delta_t = np.linalg.norm(pred_scale) * pred_RT[:3, :3] @ T0_mug
...
pred_results_total = mmcv.load('/data/zrd/RBP_result/0220_ssp/all_pred_result.pkl')

pred_results_dualposenet = mmcv.load('/data/zrd/project/RBP_result/dualposenet_results/REAL275_results.pkl')
pred_results_origin = mmcv.load('/data/zrd/project/RBP_result/dualposenet_results/REAL275_results.pkl')
...
save_dir = '/data/zrd/RBP_result/visualize_bbox_debug'
...
for img_path in tqdm(pred_results_total_dict.keys()):
    # img_path = 'data/real/test/scene_5/0064'
    ...
    pred_result['pred_class_ids'] = pred_result['gt_class_ids']
    pred_result['pred_bboxes'] = pred_result['gt_bboxes']
    pred_result['pred_RTs'] = pred_result['gt_RTs']
    pred_result['pred_scales'] = pred_result['gt_scales']

    # 'gt_class_ids', 'gt_bboxes', 'gt_RTs', 'gt_scales', 'pred_class_ids', 'pred_bboxes', 'pred_scores', 'gt_handle_visibility', 'pred_RTs', 'pred_scales'
    best_match_for_gt = find_best_match_for_gt(pred_result, True, img_path)
    ...
    for j in range(len(pred_result['gt_class_ids'])):
        ...
        pred_RT, pred_scale = best_match_for_gt_dualposenet[j]
        if pred_scale is not None:
            rgb = visualize_bbox(pred_scale, pred_RT, rgb, (255, 0, 0))
            rgb_dpn = visualize_bbox(pred_scale, pred_RT, rgb_dpn, (0, 255, 255))

        gt_scale = pred_result_dualposenet['gt_scales'][j]
        gt_RT = pred_result_dualposenet['gt_RTs'][j]
    ...
    cv2.imwrite(save_path+'_box_dpn.png', rgb_dpn)
