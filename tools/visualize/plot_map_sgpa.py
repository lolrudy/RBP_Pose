...
if __name__ == '__main__':
    dualposenet_path = '/data/zrd/datasets/NOCS/results/sgpa_results/REAL275_results.pkl'

    output_path = '/data/zrd/RBP_result/0221_sgpa'
    ...
    output_dpn_eval_file_path = '/data/zrd/datasets/NOCS/results/sgpa_results/REAL275_eval_results.pkl'
    if os.path.exists(output_dpn_eval_file_path):
        iou_aps, pose_aps = mmcv.load(output_dpn_eval_file_path)
    else:
        dualposenet_pred_results = mmcv.load(dualposenet_path)
        iou_aps, pose_aps = compute_degree_cm_mAP(dualposenet_pred_results, synset_names, output_path, degree_thres_list,
                                                  shift_thres_list,
                                                  iou_thres_list, iou_pose_thres=0.1, use_matches_for_pose=True)
        mmcv.dump([iou_aps, pose_aps], output_dpn_eval_file_path)
    ...
    with open(os.path.join(output_path, 'eval_all_results.txt'), 'w') as file:
        for line in messages:
            print(line)
            file.write(line+'\n')
    plot_mAP(degree_thres_list, shift_thres_list, iou_thres_list, iou_aps, pose_aps, os.path.join(output_path, 'sgpa_mAP.png'),
             suffix='SGPA')
