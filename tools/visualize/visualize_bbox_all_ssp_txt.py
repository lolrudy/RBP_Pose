...
save_txt_dir = '/data/zrd/RBP_result/txt_ssp_dpn'
...
for img_path in tqdm(img_path_select):
    ...
    best_match_for_gt_dualposenet = find_best_match_for_gt(pred_result_dualposenet, True, img_path)
    save_path = os.path.join(save_dir, img_path.replace('/', '_'))
    save_txt_path = os.path.join(save_txt_dir, img_path.replace('/', '_'))
    ...
    for j in range(len(pred_result['gt_class_ids'])):
        try:
            ...
            if pred_scale is not None:
                ...
                np.savetxt(save_txt_path + f'_our_rt_{j}.txt', pred_RT)
                np.savetxt(save_txt_path + f'_our_scale_{j}.txt', pred_scale)
            ...
            if pred_scale is not None:
                ...
                np.savetxt(save_txt_path + f'_dpn_rt_{j}.txt', pred_RT)
                np.savetxt(save_txt_path + f'_dpn_scale_{j}.txt', pred_scale)
            gt_scale = pred_result['gt_scales'][j]
            gt_RT = pred_result['gt_RTs'][j]
            gt_bbox = pred_result['gt_bboxes'][j]
            pred_bbox = pred_result['gt_bboxes'][j]
            np.savetxt(save_txt_path + f'_gt_scale_{j}.txt', gt_scale)
            np.savetxt(save_txt_path + f'_gt_rt_{j}.txt', gt_RT)
            np.savetxt(save_txt_path + f'_gt_bbox_{j}.txt', gt_bbox)
            np.savetxt(save_txt_path + f'_pred_bbox_{j}.txt', pred_bbox)
