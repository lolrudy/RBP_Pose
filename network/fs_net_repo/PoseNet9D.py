...
class PoseNet9D(nn.Module):
    ...
    def forward(self, points, obj_id, prior, PC_seman, mean_shape, gt_s=None):
        ...
        if FLAGS.train:
            ...
            recon, face_shift_delta, face_log_var, mask = self.face_recon(torch.cat((feat_obs, PC_seman), dim=2), feat_global_obs,
                                          points - points.mean(dim=1, keepdim=True), face_shift_prior)
